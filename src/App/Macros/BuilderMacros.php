...
//            logger()->debug($key, [array_key_exists($key, $params), !empty($params[$key])]);
        ...
        $_ifWhereLike = fn(array $params, string $key, ?string $field = null) => $this->when(valid($params, $key), fn($q) => $q->where($field ?? $key, 'like', "%$params[$key]%"));

        $_ifWhereLikeKeyword = fn(array $params, string $key, array $fields) => $this->when(valid($params, $key),
        ...
        $_ifHasWhereLike = fn(array $params, string $key, string $relation, ?string $field = null) => $this->when(valid($params, $key), fn($q) => $q->whereHas($relation, fn($q1) => $q->where($field ?? $key, 'like', "%$params[$key]%")));
